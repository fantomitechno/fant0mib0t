...
export const stringifyTime = (
  time: number,
  option: objectConstructor['stringifyTime'] = {lang: 'en', long: false, format: 'Y-MO-W-D-H-M-S-MS', separator: ', ', valueNull: false, suppressTag: false}
) => {
  ...
  if (option.format && typeof option.format === 'string') {
    ...
    if (values.filter(r => r).length === 0)
      return error(lang.errors['TIMES_NULL'], {type: 'stringifyTime'}, "stringifyTime(500, {format: 'Y-MO-W-D-H-M-S-MS'})");
  ...
  times = times.map((t, i) =>
    t === 0 && !option.valueNull
      ? null
      : t + (!option.suppressTag ? (option.long ? ' ' : '') + lang[option.lang ?? 'en'].times[i][!option.long ? 2 : t < 2 ? 0 : 1] : '')
  );
...
export const parseTime = (timeStr: string, option: objectConstructor['parseTime'] = {ms: true}) => {
  ...
  let time: (string | number)[] = timeStr
    .trim()
    .split(' ')
    .filter(r => r.length != 0);
