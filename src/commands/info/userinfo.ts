...
async function getMostColorImage(link: string) {
    ...
    return Object.entries(allColors).sort((a, b) => (b[1] as number) - (a[1] as number)).map(r => r[0])[0];
}
...
export default new Command(
	...
	async (client: Bot, interaction: CommandInteraction) => {
        ...
        ctx.fillText(formatTime(user.createdTimestamp, {format: "dddd Do MMMM YYYY, hh:mm:ss", lang: "en"})+"\n"+stringifyTime(Date.now() - user.createdTimestamp, {long: true, format: "Y-D-H", separator: ", ", valueNull: false, suppressTag: false, lang: "en"}), 25, 310)
        ctx.fillText(formatTime(member.joinedTimestamp ?? 0, {format: "dddd Do MMMM YYYY, hh:mm:ss", lang: "en"})+"\n"+stringifyTime(Date.now() - (member.joinedTimestamp ?? 0), {long: true, format: "Y-D-H", separator: ", ", valueNull: false, suppressTag: false, lang: "en"}), 25, 450)
        ...
        if (member.presence?.activities.find(r => r.type === "PLAYING")) {
            ...
            if (activity?.assets?.largeImage) {
                const loadSpotify = await loadImage(`https://static-cdn.jtvnw.net/previews-ttv/live_user_${activity.assets.largeImage.replace("twitch:", "")}-1280x720.jpg`)
