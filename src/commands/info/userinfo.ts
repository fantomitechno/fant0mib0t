...
export default new Command(
	...
	async (handler: typeof CommandHandler, message: Message, args: string[]) => {
        ...
        ctx.save()
        drawCircle(89, 89, canvas.width, ctx)
        ctx.clip()
        ...
        const memberTag = member.user.tag.split('#')
        ...
        if (member.user.bot) {
          let botRect = {
            ...
            w: ctx.measureText("BOT").width + 20
          }
        ...
        if (activity) {
          if (activity.type === "LISTENING") {
            ...
            if (activity.emoji) {
              ...
              let emojiCode = convert.toCodePoint(name)
        ...
        const attachment: any = new MessageAttachment(canvas.toBuffer(), "userinfo.png")
