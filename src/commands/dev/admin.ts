      ...
      if (i.user.id !== interaction.user.id) {
        ...
        if (i.customId === 'close') {
          ...
          client?.guilds.cache.map(async g => {
            ...
            await owner
              ?.send(
                `<a:red_alert:849643442514296843> Hi, I'm on one of your servers (${g.name}). My token have been leaked and my owners decided to enter the danger zone : I have to leave your server so you can't be raided by me. For more informations, go on my Discord server : <http://discord.gg/x9BMZ6z>`
              )
              .catch(_ => _);
            await g.leave();
